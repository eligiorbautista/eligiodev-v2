    ...
    <>
      <div className="bg-primary border border-accent/20 p-6 flex flex-col h-full group hover:border-accent/50 transition-all duration-300">
        ...
        <div className="flex-grow flex flex-col min-h-0">
          ...
          <p className="text-sm font-mono text-text-secondary mb-3">{project.date}</p>
          <p className="text-text-secondary mb-4 line-clamp-3">{project.description}</p>
          <div className="flex flex-wrap gap-2 mb-4">
            {project.tags.slice(0, 4).map((tag, i) => (
              <span key={i} className="text-sm font-mono bg-background text-accent/80 px-2 py-1">{tag}</span>
            ))}
            {project.tags.length > 4 && (
              <span className="text-sm font-mono text-text-secondary px-2 py-1">+{project.tags.length - 4}</span>
            )}
          </div>
        </div>
